...
- When moving a `dispatch(...)` from `afterUpdate` to `$:`, timing changes. `afterUpdate` ran after the DOM commit; `$:` runs during the flush, before the patch. A throwing consumer handler can abort that flush. If the block also sets `prevX = x` after dispatching, the throw can leave later updates dead (a throwing `on:update` once broke persistence in [`LocalStorage.svelte`](src/LocalStorage/LocalStorage.svelte)). For notification events, update internal `prev` state first, then `tick().then(() => dispatch(...))`. Snapshot values for the deferred callback (`const next = value`). See [`FileUploader.svelte`](src/FileUploader/FileUploader.svelte) and the storage components. For imperative DOM work, put the application `$:` block above the dispatch block; reactive blocks run in source order ([`Theme.svelte`](src/Theme/Theme.svelte)). Keep cancelable, user-driven dispatches synchronous.
