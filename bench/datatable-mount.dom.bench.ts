...
//
// No forced GC here — see treeview-mount.dom.bench.ts's comment. Forcing a
// real GC after every iteration over a large jsdom DOM object graph (rather
// than plain JS objects/arrays) induced real thermal throttling there;
// default batching (as dropdown.dom.bench.ts already uses) is safe.
...
group("mount DataTable, no virtualization", () => {
  for (const size of range(20, 300)) {
...
group("mount DataTable, virtualized", () => {
  for (const size of range(100, 3000)) {
    const rows = buildRows(size);
    task(`${size} rows`, () => {
      render(DataTable, {
...
task("mount DataTable, 3000 rows × 20 cols, virtualized", () => {
  render(DataTable, {
    props: {
      headers: wideHeaders,
      rows: wideRows3000,
