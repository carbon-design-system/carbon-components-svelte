 ...
 * `.bx--list-box__menu-item` in css/_fluid-list-box.scss. Same height
 * for every size; condensed fluid uses the size-based heights in
 * MENU_ITEM_HEIGHT.
 */
 ...
 * @param {"xs" | "sm" | "md" | "lg" | "xl"} [size] - The size variant
 * @param {{ fluid?: boolean }} [options] - Pass `fluid: true` for
 ...
 * `virtualize={false}` refuses however long the list runs, supplying
 * the prop at all asks for it, and with no prop a long enough list opts
 * itself in.
 *
 * Separate from the windowing update because `ComboBox` needs the
 * answer before it can work out which options that update receives.
 *
 * @param {Object} options
 * @param {ArrayLike<unknown>} options.items The consumer's own items
 *   rather than the filtered subset, windowing being a property of the
 *   collection supplied and not of what a keystroke narrows it to.
