...
> = {
  ...
  /**
   * Wrapping makes options unequal in height, so it is what turns
   * measuring on. Nothing else does: a `measured` key on `virtualize`
   * is dropped.
   */
  wrapOptions?: boolean;
  size?: MenuWindowSize;
...
> = {
  ...
  /**
   * Whether a windowing configuration was resolved, which differs from
   * the list being windowed: one below the threshold resolves a config
   * and still renders whole. `menuMaxHeight` came from the viewport
   * when this is `true`, so an overflow rule gates on this.
   */
  isWindowed: boolean;
  isMeasured: boolean;
};
...
> = {
  ...
  /**
   * Bring the option at `index` into view, on whichever of the measured
   * and fixed-height paths the menu is on.
   */
  ...
  /**
   * Note a scroll event, so the reader supersedes an outstanding
   * request.
   */
  ...
  cancelRequest(): void;
  /**
   * Forget the measurements on close; the estimate they averaged to
   * survives.
   */
  reset(): void;
...
> = {
  /**
   * The menu's scroll container, or nothing when the menu is not
   * rendered.
   */
...
/**
 * Which options a listbox menu renders and where they sit: the
 * windowing configuration, the visible slice, the measured heights and
 * the estimate that outlives them, scroll placement, and the menu's
 * maximum height.
 *
 * Builds on the offset arithmetic in `src/utils/virtualize.js` and the
 * DOM height observation in `src/utils/height-measurer.js`.
 */
