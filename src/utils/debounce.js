// @ts-check
/**
 * Debounce `callback` until `delay` ms after the last call. Exposes
 * `cancel()` and `flush()` on the returned function.
 *
 * @template {(...args: any[]) => void} Fn
 * @param {Fn} callback - Function to debounce.
 * @param {number} delay - Quiet period in milliseconds before
 *   `callback` runs.
