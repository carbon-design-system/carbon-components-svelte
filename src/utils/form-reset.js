 ...
 *
 * `reset` fires before the form restores its controls, and when the user
 * clicks a reset button, browsers run microtasks between listeners, before
 * the restore. The callback therefore runs on the next task, once the DOM
 * holds the reset values. It is skipped when any listener cancels the reset.
 *
