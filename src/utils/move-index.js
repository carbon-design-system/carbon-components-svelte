...
/**
 * Move `index` by `step` and wrap once at either end. Returns -1 for an
 * empty range. For single-step moves (`step` ±1), stepping from -1
 * lands on the last item (step +1) or first item (step -1).
 *
 * @param {number} index - Current index (may be -1 when nothing is
 *   selected).
...
/**
 * Move from `index` by `step` to the next enabled item, skipping
 * disabled ones. Returns `index` when no enabled item exists in that
 * direction. `index` may be -1.
 *
 * @template T
 * @param {object} options
 * @param {ReadonlyArray<T>} options.items
 * @param {number} options.index - Current index (may be -1).
 * @param {number} options.step - Direction/magnitude to move (e.g. 1 or
 *   -1).
 * @param {(item: T) => boolean} [options.isDisabled] - Defaults to
 *   `item.disabled`.
 * @param {boolean} [options.wrap] - Wrap at the ends. Default `true`;
 *   `false` clamps.
 * @returns {number}
 */
