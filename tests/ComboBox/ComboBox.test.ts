...
describe("ComboBox", () => {
  ...
  describe("virtualization", () => {
    ...
    it("should enable virtualization for large lists", async () => {
      ...
      // The exact number depends on containerHeight and itemHeight
      const options = screen.getAllByRole("option");
      // Should render fewer items than total (virtualization is active)
      expect(options.length).toBeLessThan(500);
      // But should render some items (at least the visible ones)
    ...
    it("should reset scroll position when menu reopens", async () => {
      ...
      // The ListBoxMenu itself is the scrollable container
      const menu = screen.getAllByRole("listbox")[1];
    ...
    it("should handle virtualization with custom item height", async () => {
      ...
      const options = screen.getAllByRole("option");
      // Should render items based on custom itemHeight
      expect(options.length).toBeGreaterThan(0);
      expect(options.length).toBeLessThan(500);
    });
