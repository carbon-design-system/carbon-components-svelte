...
describe("rangeSlice", () => {
  test("returns the inclusive range in list order", () => {
    expect(rangeSlice(items, 1, 3)).toEqual(["b", "c", "d"]);
  });

  test("handles a target before the anchor", () => {
    expect(rangeSlice(items, 3, 1)).toEqual(["b", "c", "d"]);
  });

  test("returns the single item when anchor equals target", () => {
    expect(rangeSlice(items, 2, 2)).toEqual(["c"]);
  });

  test("filters out ineligible items", () => {
